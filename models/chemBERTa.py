...
def load_data_chemberta(path, flag_test=False):
...
if __name__ == "__main__":
    ...
    # load the data
    train_df = load_data_chemberta(os.path.join(data_dir, 'augmented_downsampled2_split_train.csv'))
    valid_df = load_data_chemberta(os.path.join(data_dir, 'augmented_downsampled2_split_valid.csv'))
    test_df = load_data_chemberta(os.path.join(data_dir, 'augmented_downsampled2_split_test.csv'))
    ...
    test_dataset = load_data_chemberta(test_path, flag_test=True)
    submission_ids, submission_smiles = load_test_data(test_path)

    # PREDICTIONS
    final_predictions, raw_outputs = model.predict(test_dataset['text'].tolist())

    submission_file = os.path.join(this_dir, "chemberta_augmented_downsampling2_10epochs.csv")
