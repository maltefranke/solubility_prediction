...
if __name__ == "__main__":
    ...
    submission_dir = os.path.join(this_dir, "submissions")

    # XGBOOST
    filename_xgboost = os.path.join(
        submission_dir, "xg_boost_predictions_descriptors_weights_nonan.csv"
    )
    ...
    # CHEMBERTA
    filename_chemberta = os.path.join(submission_dir, "Chemberta.csv")
    df2 = pd.read_csv(filename_chemberta)

    x_chem = df2[["Id", "Pred"]].values

    # SCHNET
    filename_schnet = os.path.join(submission_dir, "SchNet.csv")
