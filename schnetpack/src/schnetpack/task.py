...
from schnetpack.src.schnetpack.model.base import AtomisticModel
...
class ModelOutput(nn.Module):
    ...
    def calculate_loss(self, pred, target):
        ...
        loss = self.loss_weight * self.loss_fn(
            pred[self.name], target[self.target_property].long()
        )
        return loss

    def update_metrics(self, pred, target, subset):
        for metric in self.metrics[subset].values():
            metric(pred[self.name], target[self.target_property].int())
