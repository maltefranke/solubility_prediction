...
def get_model(x, y, hyperparameters):
    ...
    model.covar_module.outputscale = (
        torch.ones_like(model.covar_module.outputscale)
        * hyperparameters["outputscale"]
    )
            ...
            try:
                with gpytorch.settings.prior_mode(True):
                    model, likelihood = get_model(
                        x, torch.Tensor(), hyperparameters
                    )
                    model.to(device)
...
if __name__ == "__main__":
    ...
    for redo_idx in range(1):
        print(
            evaluate(
                *get_batch(1000, 10, hyperparameters=hps, num_features=10000),
                use_mse=False,
                hyperparameters=hps
            )
        )
