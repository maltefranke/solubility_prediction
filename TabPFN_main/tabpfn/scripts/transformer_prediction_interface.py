...
def load_model_workflow(
    i, e, add_name, base_path, device="cpu", eval_addition=""
):
    """
...
def transformer_predict(
    ...
    **kwargs,
    ...
    def predict(
        ...
        inference_mode_call = (
            torch.inference_mode() if inference_mode else NOP()
        )
        ...
        if preprocess_transform_configuration in eval_xs_transformed:
            ...
            if eval_xs_.shape[-1] * 3 < max_features and combine_preprocessing:
                eval_xs_ = torch.cat(
                    [
                        preprocess_input(
                            eval_xs_, preprocess_transform="power_all"
                        ),
                        preprocess_input(
                            eval_xs_, preprocess_transform="quantile_all"
                        ),
                    ],
                    -1,
                )
