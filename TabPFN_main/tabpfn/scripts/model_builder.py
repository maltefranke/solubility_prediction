...
def save_model(model, path, filename, config_sample):
    ...
    torch.save(
        (model.state_dict(), None, config_sample), os.path.join(path, filename)
    )
...
def load_model(path, filename, device, eval_positions, verbose):
    ...
    config_sample["num_features_used_in_training"] = config_sample[
        "num_features_used"
    ]
    ...
    config_sample["num_classes"] = 3  # <- modified
    ...
    model = get_model(
        config_sample, device=device, should_train=False, verbose=verbose
    )
...
def get_mlp_prior_hyperparameters(config):
    ...
    if "prior_noise_std_gamma_k" in config:
        noise_std_sampler = gamma_sampler_f(
            config["prior_noise_std_gamma_k"],
...
def get_model(
    ...
    if (
        "aggregate_k_gradients" not in config
        or config["aggregate_k_gradients"] is None
    ):
        ...
        def new_get_batch(
            ...
            **kwargs,
            ...
            kwargs = {
                **extra_kwargs,
                **kwargs,
            }  # new args overwrite pre-specified args
    ...
    if (
        ...
        or (
            "nan_prob_a_reason" in config and config["nan_prob_a_reason"] > 0.0
        )
        or (
    ...
    aggregate_k_gradients = (
        1
        if "aggregate_k_gradients" not in config
        else config["aggregate_k_gradients"]
    )
    check_is_compatible = (
        False
    ...
    config["bptt_extra_samples"] = (
        config["bptt_extra_samples"]
        if "bptt_extra_samples" in config
        else None
    )
    ...
    model = train(
        ...
        extra_prior_kwargs_dict={
            ...
            ,
            "batch_size_per_gp_sample": config.get(
